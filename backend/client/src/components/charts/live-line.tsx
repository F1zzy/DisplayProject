...
export function LiveLine({
  ...
  const badgeLabel = formatValue(liveValue);
  const badgeWidth = Math.max(36, badgeLabel.length * 7.5 + 16);
  const badgeGap = 12;
  // Prefer right of tip; flip left when the pill would leave the plot.
  const badgeX =
    liveDotX + badgeGap + badgeWidth > innerWidth
      ? liveDotX - badgeGap - badgeWidth
      : liveDotX + badgeGap;
  const badgeY = Math.min(
    Math.max(liveDotY, 12),
    Math.max(12, innerHeight - 12)
  );
